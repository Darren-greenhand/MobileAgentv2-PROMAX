def get_action_prompt(instruction, gui_switch,ui_prompts,clickable_infos, width, height, keyboard, summary_history, action_history, last_summary, last_action, add_info, error_flag, completed_content, memory):
    prompt = "### Background ###\n"
    ...
    if gui_switch:
        ...
        prompt += "Sometimes there are additional UI information after the two parts: coordinates;content;class;content-desc;weather clickable;whether enable;whether focusable. "
    ...
    if keyboard:
        ...
        prompt += "Clear: Clear the input box because of the wrong content entered before, prepare to enter the correct content later\n"
    else:
...
def get_reflect_prompt(instruction, gui_switch,ui_prompts1,clickable_infos1, ui_prompts2,clickable_infos2, width, height, keyboard1, keyboard2, summary, action, add_info):
